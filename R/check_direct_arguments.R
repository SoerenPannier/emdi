...
direct_check <- function(y, 
  ...
  if (!inherits(smp_domains, "character") || length(smp_domains) != 1) {
    stop("Smp_domains must be a vector of length 1 and of class character 
          ...
          The variable needs to be contained in smp_data. See also 
          help(direct).")
  } 
  ...
  if (!(is.null(weights) || is.numeric(smp_data[[weights]]))) {
    stop(paste0(weights, " must be the name of a variable that is a numeric 
                vector."))
  }
  ...
  if (!is.null(threshold) && !(is.numeric(threshold) && length(threshold) == 1) 
    ...
    stop("threshold must be a single numeric value or a function of y and 
         weights. If it is NULL 60% of the median of the target variable is 
         selected as threshold. See also help(direct).")
  }
  ...
  if (inherits(threshold, "function") && 
    ...
    stop("The threshold function must return a single numeric value when 
         evaluated with the dependent variable and the weights, if present.")
  }
  ...
  if (var == TRUE && !(boot_type == "naive" || boot_type == "calibrate")) {
    stop("If var is set to TRUE, boot_type ''naive'' or ''calibrate'' needs to 
         be selected. See also help(direct).")
  }
  if (var == TRUE && !(is.numeric(B) && length(B) == 1)) {
    stop("If var is set to TRUE, a single value, interpreted as an integer, for 
          the number of bootstrap sample needs to be chosen. See also 
         help(direct).")
  }
    ...
    for (i in seq_len(N_custom)) {
      if (!inherits(custom_indicator[[i]], "function")) {
        stop("The elements of the list need to be functions. These Functions 
             ...
             not be needed and a threshold might not be included in the 
             indicator. For help see Example 3 in help(direct).")
      }
