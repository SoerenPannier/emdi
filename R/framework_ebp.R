...
framework_ebp <- function(fixed, pop_data, pop_domains, smp_data, smp_domains,
                          ...
                          pop_subdomains=NULL,smp_subdomains=NULL, tf) {
  ...
  fw_check1(
    ...
    aggregate_to = aggregate_to, fixed = fixed, smp_domains = smp_domains,
    smp_subdomains = smp_subdomains, threshold = threshold, weights = weights,
    pop_weights = pop_weights, tf = tf
  )
  ...
  fw_check2(
    ...
    aggregate_to = aggregate_to, aggregate_to_vec = aggregate_to_vec, tf = tf
  )
  if(tf == TRUE){
    fw_tf_check2(pop_subdomains = pop_subdomains,
  ...
  if(tf == TRUE){
    ...
    unsampled_subdoms_smpdoms <- unsampled_subdoms_smpdoms |>
      dplyr::filter(.data$dom_sampled == TRUE & .data$subdom_sampled == FALSE)
  ...
  if( tf == TRUE){
    fw_tf_check3(
      obs_subdom = obs_subdom, dist_obs_subdom = dist_obs_subdom,
      pop_subdomains = pop_subdomains, smp_subdomains = smp_subdomains)
