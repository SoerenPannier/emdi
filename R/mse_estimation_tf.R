...
parametric_bootstrap <- function(framework,
  ...
  if (boot_type == "wild") {
    res_s <- residuals(point_estim_tf$model)
    fitted_s <- fitted(point_estim_tf$model, level = 1)
  } else {
    res_s <- NULL
    fitted_s <- NULL
  }
  ...
  if (cpus > 1) {
    ...
    mses <- simplify2array(parallelMap::parallelLapply(
      ...
      B               = B,
      framework       = framework,
      lambda          = point_estim_tf$optimal_lambda,
      shift           = point_estim_tf$shift_par,
      model_par       = point_estim_tf$model_par,
      gen_model       = point_estim_tf$gen_model,
      fixed           = fixed,
...
mse_estim <- function(framework,
  ...
  bootstrap_point_estim_tf <- as.matrix(point_estim_tf(
    fixed = fixed,
    transformation =
      transformation,
    interval = interval,
  ...
  return((bootstrap_point_estim_tf - true_indicators)^2)
} # End mse_estim
