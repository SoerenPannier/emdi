...
model_par <- function(framework,
  ...
  if (is.null(framework$weights)) {
    ...
    for (d in 1:framework$N_dom_smp) {
      ...
      indep_var_ast   <- indep_smp - matrix(rep(gamma_weight[d] *
                                                  mean_indep[d,],
                                                framework$n_smp[d]),
...
monte_carlo <- function(transformation,
  ...
  # quant10s = quant25s = mediane = quant75s = quant90s = ginis = qsrs =
  # pgaps = hcrs = means <- matrix(nrow=framework$N_dom_pop, ncol=L)

  ests_mcmc <- array(dim = c(framework$N_dom_pop,
                             L,
