...
test_that("Does the fh function with a logit transformation return resonable
          results", {

            ########################## naive backtransformation ##########################
            # Current version
            fh_logit_naive_boot <- fh(fixed = MTMED ~ cash + age_ben + rent +
                                         house_allow,
                                       vardir = "Var_MTMED",
                                       combined_data = combined_data[combined_data$Var_MTMED > 0, ],
                                       domains = "Domain",
                                       method = "ml", interval = c(0, 10000000),
            ...
            # Status quo (benchmark)
            transf_logit_naive <- read.csv("FH/transf_logit_naive.csv",
                                            sep = ",",
                                            stringsAsFactors = TRUE)

            # Compare results from current version and benchmark
            # EBLUP
            expect_equal(fh_logit_naive_boot$ind[, c("Domain","FH")],
                         transf_logit_naive[, c("Domain","FH")])
            # MSE boot
            expect_equal(fh_logit_naive_boot$MSE$FH,
                         transf_logit_naive$MSE_boot)

            # Variance
            expect_equal(fh_logit_naive_boot$model$variance,
                         transf_logit_naive$variance[1])

            ############################ bc backtransformation ###########################
            # Current version
            fh_logit_bc_boot <- fh(fixed = MTMED ~ cash + age_ben + rent + house_allow,
                                    vardir = "Var_MTMED",
                                    combined_data = combined_data[combined_data$Var_MTMED > 0, ],
                                    domains = "Domain",
                                    method = "ml", interval = c(0, 10000000),
            ...
            # Status quo (benchmark)
            transf_logit_bc <- read.csv("FH/transf_logit_bc.csv", sep = ",",
                                         stringsAsFactors = TRUE)

            # Compare results from current version and benchmark
            # EBLUP
            expect_equal(fh_logit_bc_boot$ind[, c("Domain","FH")],
                         transf_logit_bc[, c("Domain","FH")])
            # MSE bootstrap
            expect_equal(fh_logit_bc_boot$MSE$FH,
                         transf_logit_bc$MSE_boot)
            # Variance
            expect_equal(fh_logit_bc_boot$model$variance,
                         transf_logit_bc$variance[1])
          })
